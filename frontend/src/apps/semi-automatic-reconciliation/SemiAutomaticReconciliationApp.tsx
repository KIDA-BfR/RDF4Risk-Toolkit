  ...
  return <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', p: { xs: 1, md: 2 }, borderRadius: 4 }}><Stack spacing={2}>
    <Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 }, borderRadius: 4, background: heroSurface, boxShadow: '0 18px 48px rgba(15,23,42,.08)' }}>
      <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2} alignItems={{ xs: 'stretch', lg: 'center' }} justifyContent="space-between">
        <Stack direction="row" spacing={1.5} alignItems="center"><ReconciliationIcon /><Stack><Typography variant="h5" component="h2">Semi-Automatic Reconciliation</Typography><Typography variant="body2" color="text.secondary">Guided workflow for provider queues, candidate review, SKOS curation, and SSSOM export</Typography></Stack></Stack>
...
function ConfigurePage({ snapshot }: { snapshot: Snapshot }) {
  ...
  const [suggestionLimit, setSuggestionLimit] = useState<number>(cfg.suggestion_slider ?? 10);
  ...
  useEffect(() => setSuggestionLimit(cfg.suggestion_slider ?? 10), [cfg.suggestion_slider]);
  ...
  return <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'minmax(0,1fr) 380px' }, gap: 2 }}><Stack spacing={2}>
    {(cfg.missing_config_alerts?.length ?? 0) > 0 && <Alert severity="warning" variant="outlined">Missing API credentials in config.yaml: {cfg.missing_config_alerts?.join(', ')}. Providers that require these keys will return no or limited results.</Alert>}
