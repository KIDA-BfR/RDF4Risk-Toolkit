  ...
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'minmax(0,1fr) 360px' }, gap: 2 }}>
      ...
      <Stack spacing={2}>
        ...
        ].map((item) => <Typography key={item} variant="body2" color="text.secondary">✓ {item}</Typography>)}<ContinueButton disabled={!data.has_table} onClick={goNext}>Continue to omission</ContinueButton></Stack></CardContent></Card>
      </Stack>
    </Box>
  );
}
...
function OmissionPage({ snapshot, emit, goNext }: { snapshot: Snapshot; emit: (event: AppEvent) => void; goNext: () => void }) {
  ...
  return <Stack spacing={2}>
    <Card variant="outlined"><CardContent><Stack spacing={1.5}>
      ...
      <ContinueButton disabled={!snapshot.data?.has_table} onClick={goNext}>Continue to preprocessing</ContinueButton>
    </Stack></CardContent></Card>
  </Stack>;
}
...
function PreprocessPage({ snapshot, emit, goNext }: { snapshot: Snapshot; emit: (event: AppEvent) => void; goNext: () => void }) {
  ...
  return <Stack spacing={2}>
    <Card variant="outlined"><CardContent><Stack spacing={2}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1}><PrepareButton onClick={prepare} disabled={!snapshot.data?.has_table}>Prepare transformations</PrepareButton><ResetButton color="inherit" onClick={() => emit({ type: 'clear_transformations' })} disabled={!prep.transformations_prepared}>Clear prepared rules</ResetButton><ContinueButton disabled={!snapshot.data?.has_table} onClick={goNext}>Continue to consolidation</ContinueButton></Stack>
    </Stack></CardContent></Card>
  </Stack>;
}
...
function GeneratePage({ snapshot, emit, goNext }: { snapshot: Snapshot; emit: (event: AppEvent) => void; goNext: () => void }) {
  return <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '360px minmax(0,1fr)' }, gap: 2 }}>
    <Stack spacing={2}>
      <Card variant="outlined"><CardContent><Stack spacing={1.3}><Typography variant="subtitle1">5. Generate / Refresh Matching Table</Typography><Typography variant="body2" color="text.secondary">Applies prepared transformations, staged consolidations, column omissions, and extracts predicate/object terms into strict SSSOM columns.</Typography><GenerateButton size="large" disabled={!snapshot.data?.has_table} onClick={() => emit({ type: 'generate_matching_table' })}>Generate / Refresh Matching Table</GenerateButton><ContinueButton variant="outlined" onClick={goNext} disabled={!snapshot.matching?.has_table}>Continue to Export</ContinueButton></Stack></CardContent></Card>
