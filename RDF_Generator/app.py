# app.py (RDF Generator backend service for the Material UI frontend)
...
try:
    ...
    try:
        from .dcat_generator import create_dcat_catalog, create_dcat_catalog_bundle, THEMES, LICENSES
    except ImportError:
        create_dcat_catalog = None
        create_dcat_catalog_bundle = None
        THEMES = {}
        LICENSES = {}
except ImportError as e:
    raise RuntimeError(f"Fatal Error: Could not import RDF Generator backend functions. {e}") from e
# --- End Imports ---
...
def read_uploaded_file(file, header_row, csv_separator=None, sheet_name=None):
    ...
    try:
        ...
        if file.name.endswith('.csv'):
            ...
            logging.error("Unsupported file type: %s", getattr(file, "name", "<unknown>"))
            return None
        ...
        logging.error("Error reading file '%s' starting at row %s: %s", getattr(file, "name", "<unknown>"), header_row, e)
        return None
...
def generate_ids(df, id_column_name, prefix):
    ...
    if id_column_name in df.columns:
        ...
        logging.warning("Generated ID column name conflicted. Using '%s' instead.", id_column_name)
    ...
    for key, value in defaults.items():
        if key not in runtime_state:
            if isinstance(value, (dict, list)):
                runtime_state[key] = value.copy()
            else:
                runtime_state[key] = value
    config = runtime_state.get("config") or {}
    if not runtime_state.get("rdf_gen_graph_base_uri"):
        runtime_state["rdf_gen_graph_base_uri"] = config.get("default_namespace", "http://example.com/data/")
    if not runtime_state.get("rdf_gen_dcat_publisher_uri"):
        default_ns = config.get("default_namespace", "http://example.com/data/")
        runtime_state["rdf_gen_dcat_publisher_uri"] = f"{str(default_ns).rstrip('/')}/organization"


def _set_rdf_status(severity: str, text: str) -> None:
    runtime_state[RDF_STATUS_MESSAGE_KEY] = {"severity": severity, "text": text}
...
def _template_download_json() -> str:
    return json.dumps(runtime_state.get("schema_templates", []), indent=2)
...
def _load_bytes_as_dataframe(kind: str) -> None:
    ...
    file_bytes = runtime_state.get(bytes_key)
    filename = runtime_state.get(name_key)
    ...
    df = read_uploaded_file(
        file_obj,
        int(runtime_state.get(header_key, 1) or 1),
        runtime_state.get(sep_key),
        runtime_state.get(sheet_key),
    )
    if df is None:
        runtime_state[df_key] = None
    ...
    if kind == "mapping":
        ...
        if missing:
            ...
            return
    runtime_state[df_key] = df
    sheet_note = f" (sheet: {runtime_state.get(sheet_key)})" if runtime_state.get(sheet_key) else ""
    runtime_state[msg_key] = f"Loaded {'Data Table' if kind == 'data' else 'Matching Table'} from uploaded file: {filename}{sheet_note}."
    _set_rdf_status("success", runtime_state[msg_key])
...
def _handle_upload(kind: str, event: Dict[str, Any]) -> None:
    ...
    runtime_state[bytes_key] = file_bytes
    runtime_state[name_key] = filename
    sheets = get_excel_sheets(_NamedBytesIO(file_bytes, filename)) if filename.lower().endswith((".xlsx", ".xls")) else []
    runtime_state[sheets_key] = sheets
    runtime_state[sheet_key] = sheets[0] if sheets else None
    if event.get("header_row"):
        runtime_state["rdf_gen_header_row_main" if kind == "data" else "rdf_gen_header_row_mapping"] = max(1, int(event.get("header_row") or 1))
    _load_bytes_as_dataframe(kind)


def _load_shared_data() -> None:
    candidate = runtime_state.get("shared_preprocessed_data")
    if isinstance(candidate, pd.DataFrame):
        runtime_state["rdf_gen_data_df"] = candidate.copy()
        runtime_state["rdf_gen_data_source_msg"] = "Loaded Preprocessed Data from Matching Table Generator."
        runtime_state[RDF_DATA_FILE_NAME_KEY] = "shared_preprocessed_data"
        _set_rdf_status("success", runtime_state["rdf_gen_data_source_msg"])
    else:
        _set_rdf_status("warning", "No preprocessed data from the Matching Table Generator is available.")


def _load_shared_mapping() -> None:
    map_df_candidate = runtime_state.get("shared_reconciled_matching_table")
    ...
    all_original_terms = runtime_state.get("all_terms_for_reconciliation")
    ...
    runtime_state["rdf_gen_mapping_df"] = current_mapping_df.copy()
    runtime_state["rdf_gen_mapping_source_msg"] = "Loaded Reconciled Matching Table from Reconciliation App (prefilled 'No Match' entries)."
    runtime_state[RDF_MAPPING_FILE_NAME_KEY] = "shared_reconciled_matching_table"
    _set_rdf_status("success", runtime_state["rdf_gen_mapping_source_msg"])


def _suggested_graph_uri() -> str:
    config = runtime_state.get("config", {}) or {}
    graph_base = str(runtime_state.get("rdf_gen_graph_base_uri") or config.get("default_namespace", "http://example.com/data/"))
    filename = str(runtime_state.get(RDF_DATA_FILE_NAME_KEY) or "graph_data")
...
def _current_config_state() -> Dict[str, Any]:
    df = runtime_state.get("rdf_gen_data_df")
    mapping_df = runtime_state.get("rdf_gen_mapping_df")
    ...
    if not runtime_state.get("map_term_col") and default_term:
        runtime_state["map_term_col"] = default_term
    if not runtime_state.get("map_uri_col") and default_uri:
        runtime_state["map_uri_col"] = default_uri
    if not runtime_state.get("map_match_col") and default_match:
        runtime_state["map_match_col"] = default_match
    graph_option = runtime_state.get("rdf_gen_graph_option", "Use Named Graph")
    named_graph = runtime_state.get("ng_full") or _suggested_graph_uri()
    return {
        "data_columns": data_cols,
        "mapping_columns": map_cols,
        "id_option": runtime_state.get("rdf_gen_id_option", "Existing ID Column"),
        "default_id_col": runtime_state.get("rdf_gen_default_id_col", ""),
        "use_shared_id": bool(runtime_state.get("rdf_gen_use_shared_id", False)),
        "shared_id_col": runtime_state.get("rdf_gen_shared_id_col", ""),
        "subject_base_uri": runtime_state.get("rdf_gen_subject_base_uri", ""),
        "graph_option": graph_option,
        "graph_base_uri": runtime_state.get("rdf_gen_graph_base_uri", ""),
        "named_graph_uri": named_graph,
        "term_col": runtime_state.get("map_term_col", default_term),
        "uri_col": runtime_state.get("map_uri_col", default_uri),
        "match_type_col": runtime_state.get("map_match_col", default_match),
        "use_class": bool(runtime_state.get("rdf_gen_use_class", False)),
        "class_uri": runtime_state.get("rdf_gen_class_uri", ""),
        "rdf_format": runtime_state.get("rdf_gen_format", "Turtle"),
        "active_template": runtime_state.get("rdf_gen_active_schema_template", "None (use default column mapping)"),
        "group_active": bool(runtime_state.get("rdf_gen_group_active", False)),
        "group_suggestions": runtime_state.get("rdf_gen_group_suggestions", {}),
        "group_config": runtime_state.get("rdf_gen_group_config", {}),
    }
...
def _build_rdf_snapshot() -> Dict[str, Any]:
    ...
    file_stem = os.path.splitext(str(runtime_state.get(RDF_DATA_FILE_NAME_KEY) or "rdf_output"))[0] or "rdf_output"
    previews = {
        "rdf": (runtime_state.get("rdf_data") or "")[:5000],
        "skos": (runtime_state.get("skos_data") or "")[:5000],
        "dcat_metadata": (runtime_state.get("dcat_metadata_data") or "")[:5000],
        "dcat_catalog": (runtime_state.get("dcat_catalog_data") or "")[:5000],
        "reference": (runtime_state.get("reference_rdf") or "")[:5000],
    }
    return {
        "active_stage": runtime_state.get("rdf_generator_active_stage", "load"),
        "statusMessage": runtime_state.get(RDF_STATUS_MESSAGE_KEY),
        "data": {
            ...
            "filename": runtime_state.get(RDF_DATA_FILE_NAME_KEY, ""),
            "source_message": runtime_state.get("rdf_gen_data_source_msg"),
            "shared_available": isinstance(runtime_state.get("shared_preprocessed_data"), pd.DataFrame),
        ...
        "mapping": {
            ...
            "filename": runtime_state.get(RDF_MAPPING_FILE_NAME_KEY, ""),
            "source_message": runtime_state.get("rdf_gen_mapping_source_msg"),
            "shared_available": isinstance(runtime_state.get("shared_reconciled_matching_table"), pd.DataFrame),
            "preview": _records(mapping_df, 50),
        },
        "load_options": {
            "data_header_row": int(runtime_state.get("rdf_gen_header_row_main", 1) or 1),
            "mapping_header_row": int(runtime_state.get("rdf_gen_header_row_mapping", 1) or 1),
            "data_csv_separator": runtime_state.get("rdf_gen_data_csv_separator"),
            "mapping_csv_separator": runtime_state.get("rdf_gen_mapping_csv_separator"),
            "data_sheets": runtime_state.get("rdf_gen_data_sheets", []),
            "mapping_sheets": runtime_state.get("rdf_gen_mapping_sheets", []),
            "data_sheet": runtime_state.get("rdf_gen_data_sheet"),
            "mapping_sheet": runtime_state.get("rdf_gen_mapping_sheet"),
        },
        "templates": {
            "items": runtime_state.get("schema_templates", []),
        ...
        "reference": {
            "loaded": runtime_state.get("reference_data") is not None,
            "method": runtime_state.get("rdf_gen_reference_method", "DOI"),
            "doi": runtime_state.get("rdf_gen_doi_input", ""),
        ...
        "outputs": {
            "has_rdf": bool(runtime_state.get("rdf_data")),
            "has_skos": bool(runtime_state.get("skos_data")),
            "has_dcat_metadata": bool(runtime_state.get("dcat_metadata_data")),
            "has_dcat_catalog": bool(runtime_state.get("dcat_catalog_data")),
            "has_reference": bool(runtime_state.get("reference_rdf")),
            "file_stem": file_stem,
            "rdf_data": runtime_state.get("rdf_data") or "",
            "skos_data": runtime_state.get("skos_data") or "",
            "dcat_metadata_data": runtime_state.get("dcat_metadata_data") or "",
            "dcat_catalog_data": runtime_state.get("dcat_catalog_data") or "",
            "reference_rdf": runtime_state.get("reference_rdf") or "",
            "previews": previews,
        },
        "dcat": {
            "available": bool(runtime_state.get("rdf_data") and runtime_state.get("last_named_graph_uri")),
            "title": runtime_state.get("rdf_gen_dcat_title", "My Dataset"),
            "description": runtime_state.get("rdf_gen_dcat_description", "An example dataset."),
            "access_rights": runtime_state.get("rdf_gen_dcat_access_rights", "PUBLIC"),
            "contact_point": runtime_state.get("rdf_gen_dcat_contact_point", ""),
            "publisher_name": runtime_state.get("rdf_gen_dcat_publisher_name", "My Organization"),
            "publisher_uri": runtime_state.get("rdf_gen_dcat_publisher_uri", ""),
            "themes": runtime_state.get("rdf_gen_dcat_themes", []),
            "theme_options": list(THEMES.keys()),
            "license": runtime_state.get("rdf_gen_dcat_license", next(iter(LICENSES.keys()), "CC BY 4.0")),
            "license_options": list(LICENSES.keys()),
            "link_reference": bool(runtime_state.get("rdf_gen_dcat_link_reference", True)),
        },
    }


def _reference_summary() -> str:
    ref = runtime_state.get("reference_data")
...
def _apply_settings(settings: Dict[str, Any]) -> None:
    ...
    for source, target in key_map.items():
        if source in settings:
            runtime_state[target] = settings[source]
    ...
    try:
        ...
        for col in ["label", "ui_link", "acronym", "source", "message"]:
            ...
            if enriched_col in merged.columns:
                ...
                merged[col] = ""
        runtime_state["rdf_gen_mapping_df"] = merged
...
def _generate_reference(event: Dict[str, Any]) -> None:
    method = str(event.get("method") or runtime_state.get("rdf_gen_reference_method", "DOI"))
    runtime_state["rdf_gen_reference_method"] = method
    try:
        converter = DOIToSemOpenAlexConverter()
        if method == "DOI":
            doi = str(event.get("doi") or runtime_state.get("rdf_gen_doi_input", "")).strip()
            if not doi:
            ...
            if not graph:
                ...
                return
            runtime_state["reference_data"] = {"method": "DOI", "doi": doi, "graph": graph}
            runtime_state["reference_rdf"] = converter.serialize(format="turtle")
            runtime_state["rdf_gen_doi_input"] = doi
        ...
        runtime_state["reference_data"] = {"method": "Manual", "metadata": manual_metadata, "graph": converter.graph}
        runtime_state["reference_rdf"] = converter.serialize(format="turtle")
...
def _generate_rdf() -> None:
    ...
    if issues:
        ...
        return
    df = runtime_state.get("rdf_gen_data_df")
    mapping_df = sync_matching_table_schemas(runtime_state.get("rdf_gen_mapping_df"))
    runtime_state["rdf_gen_mapping_df"] = mapping_df
    cfg = _current_config_state()
    config = runtime_state.get("config", {}) or {}
    ...
    try:
        ...
        rdf_graph = create_rdf_with_mappings(
            ...
            schema_templates=runtime_state.get("schema_templates", []),
            active_template_name=active_template,
            config=config,
            term_to_concept_uri_map=term_to_uri_lookup,
            input_data_path=str(runtime_state.get(RDF_DATA_FILE_NAME_KEY) or "default_filename"),
            original_column_order=list(df.columns),
        )
        runtime_state["rdf_graph"] = rdf_graph
        runtime_state["skos_graph"] = skos_graph
        runtime_state["rdf_data"] = rdf_graph.serialize(format="turtle")
        runtime_state["skos_data"] = skos_graph.serialize(format="turtle")
        runtime_state["dcat_catalog_data"] = None
        runtime_state["dcat_metadata_data"] = None
        runtime_state["last_named_graph_uri"] = named_graph_uri
        runtime_state["last_rdf_format_display"] = cfg["rdf_format"]
        _set_rdf_status("success", "RDF data graph and SKOS vocabulary generated successfully.")
    except Exception as exc:
        runtime_state["rdf_data"] = None
        runtime_state["skos_data"] = None
        _set_rdf_status("error", f"Error during RDF generation: {exc}")


def _generate_dcat() -> None:
    if create_dcat_catalog_bundle is None:
        _set_rdf_status("error", "DCAT catalog generation is not available.")
        return
    rdf_graph = runtime_state.get("rdf_graph")
    skos_graph = runtime_state.get("skos_graph")
    named_graph_uri = runtime_state.get("last_named_graph_uri")
    ...
    if not default_namespace:
        ...
        return
    rdf_format_display = runtime_state.get("last_rdf_format_display", "Turtle")
    format_map = {"Turtle": "turtle", "N-Quads": "nquads", "JSON-LD": "json-ld", "RDF/XML": "pretty-xml", "TriG": "trig"}
    reference_data = runtime_state.get("reference_data")
    metadata_config = {
        "title": runtime_state.get("rdf_gen_dcat_title", "My Dataset"),
        "description": runtime_state.get("rdf_gen_dcat_description", "An example dataset."),
        "keywords": [],
        ...
        "creator": [],
        "access_rights": runtime_state.get("rdf_gen_dcat_access_rights", "PUBLIC"),
        "contact_point": str(runtime_state.get("rdf_gen_dcat_contact_point", "") or "").strip(),
        "publisher_name": runtime_state.get("rdf_gen_dcat_publisher_name", "My Organization"),
        "publisher_uri": runtime_state.get("rdf_gen_dcat_publisher_uri", f"{str(default_namespace).rstrip('/')}/organization"),
        "themes": runtime_state.get("rdf_gen_dcat_themes", []),
        "license": runtime_state.get("rdf_gen_dcat_license", next(iter(LICENSES.keys()), "CC BY 4.0")),
        "link_reference": bool(reference_data and runtime_state.get("rdf_gen_dcat_link_reference", True)),
        "reference_data": reference_data if reference_data and runtime_state.get("rdf_gen_dcat_link_reference", True) else None,
    }
    try:
        catalog_trig, metadata_ttl = create_dcat_catalog_bundle(
            rdf_graph=rdf_graph,
        ...
        runtime_state["dcat_catalog_data"] = catalog_trig
        runtime_state["dcat_metadata_data"] = metadata_ttl
...
def _handle_templates_event(event: Dict[str, Any]) -> None:
    ...
    templates = list(runtime_state.get("schema_templates", []))
    if action == "load_json":
        try:
            ...
            runtime_state["schema_templates"] = loaded
        ...
        runtime_state["schema_templates"] = templates
...
def _handle_rdf_mui_event(event: Any) -> bool:
    ...
    if nonce and runtime_state.get(RDF_COMPONENT_ACTION_NONCE_KEY) == nonce:
        return False
    if nonce:
        runtime_state[RDF_COMPONENT_ACTION_NONCE_KEY] = nonce
    event_type = str(event.get("type", "") or "")
    should_rerun = True
    if event_type == "navigate":
        runtime_state["rdf_generator_active_stage"] = str(event.get("stage", "load") or "load")
        ...
        df = runtime_state.get("rdf_gen_data_df")
        if isinstance(df, pd.DataFrame):
            runtime_state["rdf_gen_group_suggestions"] = suggest_groups(df.columns)
            _set_rdf_status("success", f"Detected {len(runtime_state['rdf_gen_group_suggestions'])} potential column group(s).")
        else:
...
def main():
    ...
    """
    raise SystemExit(
        "RDF_Generator.app is a backend service. Start the app with `npm start` "
